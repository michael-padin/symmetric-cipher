    ...
    return text
      ...
      .map((char) =>
        char.match(/[a-z]/i) // If the character is a letter
          ? String.fromCharCode(
              ((char.charCodeAt(0) - (char <= "Z" ? 65 : 97) + shift) % 26) + (char <= "Z" ? 65 : 97)
...
export const vigenereCipher = {
  ...
  encrypt: (text: string, key: string) => {
    ...
    let result = "";
    const extendedKey = cleanKey.repeat(Math.ceil(cleanMessage.length / cleanKey.length)).slice(0, cleanMessage.length); // Extend the key to match the message length
  ...
  decrypt: (text: string, key: string) => {
    ...
    let result = "";
    const extendedKey = cleanKey.repeat(Math.ceil(cleanCiphertext.length / cleanKey.length)).slice(0, cleanCiphertext.length); // Extend the key to match the ciphertext length
...
export const playfairCipher = {
  ...
  createTable: (key: string) => {
    ...
    key = key.toUpperCase().replace(/[^A-Z]/g, "").replace(/J/g, "I").split("").join(""); // Clean the key and replace 'J' with 'I'
    const used = new Set();
  ...
  encrypt: (text: string, key: string) => {
    ...
    text = text.toUpperCase().replace(/[^A-Z]/g, "").replace(/J/g, "I"); // Clean the text
...
export const columnarCipher = {
  ...
  decrypt: (encrypted: string, key: string) => {
    ...
    const plainText = decrypted.replace(/_/g, " "); // Restore spaces
